...
if [ -n "$bind_address" ]; then
	HTTP_REQ="wget-ssl --bind-address=$bind_address --no-check-certificate -O - "
	POST_ARG="--post-data="
else
	HTTP_REQ="wget-ssl --no-check-certificate -O - "
	POST_ARG="--post-data="
fi
...
get_kuainiao_api(){
	...
	if [ -z "$portal_ip" ]
		then
			kuainiao_warning="迅雷快鸟服务API获取失败，请检查网络环境，或稍后再试!"
...
if [[ ! $kuainiao_can_upgrade -eq 1 ]]; then
	kuainiao_run_i=6
	echo "您的宽带不能使用讯鸟快鸟加速！"$(date "+%Y-%m-%d %H:%M:%S")
	kuainiao_run_status=0
	exit 21
fi
...
if test $kuainiao_run_i -ge 6; then
	...
	if [ -z "$session" ]; then
		#登陆失败重置计数器到6
		kuainiao_run_i=6
		kuainiao_run_warnning="迅雷账号登陆失败！请检查迅雷账号配置！"$(date "+%Y-%m-%d %H:%M:%S")
		kuainiao_run_status=0
		echo "迅雷账号登陆失败！请检查迅雷账号配置！"$(date "+%Y-%m-%d %H:%M:%S")
		exit 20
	else
		#登陆成功设置登陆日期和session
		day_of_month_orig=`date +%d`
...
if [ ! -z "`echo $ret|grep "not exist channel"`" ]; then
	kuainiao_run_i=6
	echo "迅雷快鸟心跳保持失败！"$(date "+%Y-%m-%d %H:%M:%S")
	kuainiao_run_status=0
	exit 22
else
	kuainiao_run_i=$(expr $kuainiao_run_i + 1)
	echo "迅雷快鸟运行正常！"$(date "+%Y-%m-%d %H:%M:%S")
